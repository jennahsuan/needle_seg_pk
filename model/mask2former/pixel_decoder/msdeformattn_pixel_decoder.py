# mmdet/models/layers/msdeformattn_pixel_decoder.py
...
from .mask2former_layers import Mask2FormerTransformerEncoder
...
class MSDeformAttnPixelDecoder(BaseModule):
    ...
    def __init__(self,
        ...
        self.num_encoder_levels = 3  # encoder.layer_cfg.self_attn_cfg.num_levels
        ...
        self.encoder = Mask2FormerTransformerEncoder(num_layers=6,  # Number of encoder layers
                                                    layer_cfg=dict(  # Configuration for each layer in the encoder
                                                        self_attn_cfg=dict(  # MultiScaleDeformableAttention configuration
                                                            embed_dims=256,
                                                            num_heads=8,
                                                            num_levels=3,
                                                            num_points=4,
                                                            ...
                                                            norm_cfg=None,
                                                            init_cfg=None),
                                                        ffn_cfg=dict(  # Feed-Forward Network configuration
                                                            embed_dims=256,
                                                    ...
                                                    init_cfg=None)
        self.postional_encoding = SinePositionalEncoding(num_feats=128, normalize=True)
    ...
    def forward_features(self, feats: List[Tensor]) -> Tuple[Tensor, Tensor]:
        """
        Args:
        ...
        for i in range(self.num_input_levels - self.num_encoder_levels - 1, -1,
            ...
            outs.append(y)
        multi_scale_features = outs #[:self.num_outs]  ## NOTE keep all four scale from small to big!

        mask_feature = self.mask_feature(outs[-1])
        return mask_feature, outs[0], multi_scale_features