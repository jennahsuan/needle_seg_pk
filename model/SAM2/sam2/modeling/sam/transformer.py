...
class Attention(nn.Module):
    ...
    def _separate_heads(self, x: Tensor, num_heads: int) -> Tensor:
        """
        B, HW, C -> B, N_heads, HW, C_per_head
        """
        b, n, c = x.shape
    ...
    def _recombine_heads(self, x: Tensor) -> Tensor:
        """
        b, n_heads, n_tokens, c_per_head -> b, n_tokens, n_heads * c_per_head
        """
...
class RoPEAttention(Attention):
    ...
    def forward(
        ...
        """
        Input [B, H*W, C_q] [B, MHW, C_kv] [B, MHW, C_kv]
        will project to same C 
        """
        # Input projections
        q = self.q_proj(q)
        k = self.k_proj(k)
...
class LocalRoPEAttention(RoPEAttention):
    """Attention with rotary position encoding.
        Local style based on deAOT."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.d_att = self.embedding_dim // self.num_heads
        self.d_middle = self.d_att * self.num_heads
    ...
    def forward(
        ...
        ### relative_emb_k from LocalGatedPropagation in deAOT
        q_unflatten = q.permute(0,2,1).unflatten(-1,(h,w))  ## B,C,H,W
        relative_emb = self.relative_emb_k(q_unflatten)
        relative_emb = relative_emb.view(batch_size, self.num_heads,
        ...
        q = self._separate_heads(q, self.num_heads)
        k = self._separate_heads(k, self.num_heads)
        v = self._separate_heads(v, self.num_heads)  ## B, N_heads, HW, C_per_head
        ...
        # ---------------------------------------------
        ### From LocalGatedPropagation in deAOT
        ...
        q = q_unflatten.unflatten(1,(self.num_heads,self.d_att)) ## B,C,H,W -> B,N_head,C_per_head,H,W
        q = q.flatten(0,1)   ## Q: B*N_head,C_per_head,H,W
        k_h_w = k.permute(0,1,3,2).flatten(0,1).unflatten(-1,(h,w)) ## B*N_heads,C_per_head,H,W
        if self.enable_corr:
            qk = self.correlation_sampler(q, k_h_w).view(
                batch_size, self.num_heads, self.window_size * self.window_size, h * w)
        else:
            unfolded_k = self.pad_and_unfold(k_h_w).view(
                batch_size*self.num_heads, self.d_att,
                self.window_size*self.window_size, h, w)  ## K: B*N_heads,C_per_head,p*p,H,W
        ...
        # print(f'[local->global_attn] {global_attn.shape} v {v.shape} ')
        out = global_attn @ v  ## [B, Nhead, HW, C_per_head]
        # ---------------------------------------------
    ...
    def local2global(self, local_attn, height, width):
        ...
        global_attn = torch.zeros(
            (batch_size, self.num_heads, height * width, pad_height, pad_width),
            device=local_attn.device)
        global_attn[local_mask.expand(batch_size, self.num_heads,
